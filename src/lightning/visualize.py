...
def visualize_input_and_output(raw_input,un_normalized_transformed_input,infered_prediction):
    fig, axes = plt.subplots(1, 3)
    axes[0].imshow(raw_input)
    axes[0].set_title('raw_input')
    axes[1].imshow(un_normalized_transformed_input)
    axes[1].set_title('un_normalized_transformed_input')
    axes[2].imshow(infered_prediction)
    axes[2].set_title('infered_prediction')
    plt.show()
...
def visualize_dataset(dataset,experiment_settings):
    """
    ...
    for sample_idx in range(nr_of_visualizations):
        for augmetnation_examples in range(3):
            visualize_input_and_output(raw_input, un_normalized_transformed_input, infered_prediction)
